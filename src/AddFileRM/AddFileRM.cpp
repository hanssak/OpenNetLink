	...
	UINT  uFlags )
{
	...
	if(m_bData==true)
	{
		InsertMenu ( hmenu, uMenuIndex, MF_BYPOSITION, uID++, _T("파일전송시스템 파일추가") );
	...
	UINT  cchMax )
{
	...
	{
		LPCTSTR szText = _T("파일전송시스템 파일추가");
...
HRESULT CAddFileRM::InvokeCommand ( LPCMINVOKECOMMANDINFO pCmdInfo )
{
	...
	{	
		...
		if(RegOpenKeyExW(HKEY_CLASSES_ROOT, _T("CLSID\\{77265A4A-E102-431F-9F6F-6050237F817C}\\InprocServer32"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) 
		{
		...
		wsprintf (szListFilePath, _T("%s\\%s.txt"),szRListPath, sztemp);
