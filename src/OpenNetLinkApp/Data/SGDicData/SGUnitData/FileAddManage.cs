...
namespace OpenNetLinkApp.Data.SGDicData.SGUnitData
{
    ...
    public class FileAddManage
    {
        ...
        public bool GetReasonAndDisplayOfErrSource(out List<(string reason, string count)> ListReason, out List<FileAddErr> ListDisaplayErrSource)
        {
            ...
            // 동시접근 error 발생회피
            List<FileAddErr> FileAddErrList = new List<FileAddErr>();
            FileAddErrList.AddRange(m_FileAddErrList);
        ...
        private bool getReasonAndDisplaySource(List<FileAddErr> getFileAddErrList, ref Dictionary<eFileAddErr, int> getFileAddErrReason)
        {
            ...
            /*try
            {
                int nListCount = (getFileAddErrList?.Count ?? 0);
                for (int nIdx = 0; nIdx < nListCount; nIdx++)
                {
                    if (getFileAddErrList[nIdx].eErrType != eFileAddErr.eFANone)
                    {
                        includeErr = true;

                        //Tree에 표시할 사유 세팅
                        getFileAddErrList[nIdx].ExceptionReason = SetExceptionReason(getFileAddErrList[nIdx].eErrType);
                        int errCnt = getFileAddErrReason.FirstOrDefault(i => i.Key == getFileAddErrList[nIdx].eErrType).Value;
                        getFileAddErrReason[getFileAddErrList[nIdx].eErrType] = ++errCnt;
                    }

                    //하위 파일 Err 확인
                    if (getFileAddErrList[nIdx].ChildrenFiles != null && getFileAddErrList[nIdx].ChildrenFiles.Count > 0)
                    {
                        bool hasChildrenErr = getReasonAndDisplaySource(getFileAddErrList[nIdx].ChildrenFiles, ref getFileAddErrReason);
                        getFileAddErrList[nIdx].HasChildrenErr = hasChildrenErr;
                        includeErr = includeErr || hasChildrenErr;
                    }
                }
            }
            ...
            {
                Log.Logger.Here().Error($"getReasonAndDisplaySource, Exception(MSG) : {ex.Message}");
            }*/
