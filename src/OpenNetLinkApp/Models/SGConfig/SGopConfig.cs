...
namespace OpenNetLinkApp.Models.SGConfig
{
    public class SGopConfig : ISGopConfig
    {
        ...
        public int CustomLoginType { get; set; } = 0;                              //사이트별 로그인 방식 커스터마이징
        ...
        public string strLoginTextFontSize { get; set; } = "1.4rem";             //로그인 화면 망연계솔루션 TEXT FontSize
        ...
        //파일 전송
        public bool bUseFileTrans { get; set; } = true;                             //파일 전송 사용 유무(메뉴바 사용 유무)
        ...
        public bool bUseEmailApprovePreviewPopup { get; set; } = false;       //메일 결재 미리보기 팝업 사용 (Default :false)
        ...
        public bool bUserLineShowNameAndID { get; set; } = false;                           // 결재라인 또는 수신자라인 등에[이름 & 직급] 포맷이 아닌, [이름&아이디]        포맷을 사용(Default:false)
        ...
        //부가 기능 (Tray, 자동 실행 등)
        public bool bViewOnlyToName { get; set; } = false;                                  //Left 사이드 메뉴바에 Group 명을 To Name으로만 보이게
        public bool bViewOnlyFromName { get; set; } = false;                                  //Left 사이드 메뉴바에 Group 명을 From Name으로만 보이게
        public bool bUseAllMenuExpand { get; set; } = false;                                //Left 사이드 메뉴바 전부 Expand하게 할 지 여부
        ...
        public bool bHideHeaderAlram { get; set; } = false;                    //우측 상단 Header의 아이콘에서 알람 Hide 여부 - 기본 false - 보여준다.
    }
