    ...
    public class HSCmdCenter
    {
        ...
        private void SGDataRecvAdvanced(int groupId, eAdvancedCmdList cmd, SGData sgData)
        {
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case eAdvancedCmdList.ePostApproversRetrieval:      // 조건별 결재자 검색
                        hs = GetConnectNetWork(groupId);
                        if (hs != null)
                        {
                            sgDicRecvData.SetDeptApprLineSearchData(hs, groupId, sgData);
        ...
        public int RestSendApproveBatch(int groupid, Int64 strUserSeq, string strApproveType, string strDataType, string strApproveAction, string strstrDescription, List<Int64> listSeqData)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RequestRestSendApproveBatch-Task-Exception : {ex.Message}");
                }
        ...
        public int RestSendForwardCancel(int groupid, string strTransSeq)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RequestRestSendForwardCancel-Task-Exception : {ex.Message}");
                }
        ...
        public int RestSendBoardNotiConfirm(int groupid, List<Int64> listBoardSeq)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RequestRestSendBoardNotiConfirm-Task-Exception : {ex.Message}");
                }
        ...
        public int RestDashBoardData(int groupid, string strFromToDate, int nItemCount)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RequestRestDashBoardData-Task-Exception : {ex.Message}");
                }
        ...
        public int RestUserSearch(int groupid, string strApproverName, string strDeptSeq, string strDeptName, UserApproveRightType ApproveType)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RestSendDeptApprLineSearch-Task-Exception : {ex.Message}");
                }
        ...
        public int RestSendUrlRedirection(int groupid, int nTotalCount, int nCurrentCount, int nSubDataType, string strUrlData)
        {
            HsNetWork hsNetWork = null;
            hsNetWork = GetConnectNetWork(groupid);
            if (hsNetWork == null)
        ...
        public int RestAgentBlockReason(int groupid, List<AgentBlockData> listAgentBlockData)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Log.Logger.Here().Error($"RequestRestAgentBlockReason-Task-Exception : {ex.Message}");
                }
        ...
        /// <param name="groupid"></param>
        /// <returns></returns>
        public int RestInstApproveClear(int groupid, eCmdReponseType reponseType = eCmdReponseType.CallBackPosResponse)
        {
            HsNetWork hsNetWork = null;
            hsNetWork = GetConnectNetWork(groupid);
            if (hsNetWork == null)
            ...
            if (reponseType == eCmdReponseType.CallPosReponse)
            {
                Task.Run(() =>
                {
                    try
                    {
                        ret = sgSendData.RequestRestInstApproveClear(hsNetWork, reponseType);
                        if (ret < 0)
                        {
        ...
        /// </summary>
        /// <param name="groupid"></param>
        /// <param name="listProxyApprover"></param>
        /// <returns></returns>
        ...
        /// <param name="groupid"></param>
        /// <returns></returns>
        public int RestInstApproveListGet(int groupid)
        {
            HsNetWork hsNetWork = null;
            hsNetWork = GetConnectNetWork(groupid);
            if (hsNetWork == null)
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    ret = sgSendData.RequestRestInstApproveListGet(hsNetWork);
                    if (ret < 0)
                    {
                        Log.Logger.Here().Error($"RequestRestInstApproveListGet-Task-Ret : {ret}");
                    }
                }
                catch (Exception ex)
                {
                    Log.Logger.Here().Error($"RequestRestInstApproveListGet-Task-Exception : {ex.Message}");
                }
        ...
        public int RestSendDeptInfo(int groupid)
        {
            HsNetWork hsNetWork = null;
            hsNetWork = GetConnectNetWork(groupid);
            if (hsNetWork == null)
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    ret = sgSendData.RequestRestSendDeptInfo(hsNetWork);
                    if (ret < 0)
                    {
                        Log.Logger.Here().Error($"RequestRestSendDeptInfo-Task-Ret : {ret}");
                    }
                }
                catch (Exception ex)
                {
                    Log.Logger.Here().Error($"RequestRestSendDeptInfo-Task-Exception : {ex.Message}");
                }
