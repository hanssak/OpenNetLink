...
return query 
	...
	SELECT 'FILE',(F.FILE_SEQ||'|'||F.DLP||'|'||F.FILE_NAME||'|'||F.FILE_KIND||'|'||F.FILE_SIZE||'|'||(CASE WHEN VIRUS_FLAG='1' THEN VIRUS_DESC ELSE '-' END) || '|'|| (CASE WHEN VIRUS_FLAG='1' THEN '-' ELSE V.VIRUS_DESC END) ) 
	FROM TBL_EMAIL_ADD_FILE F 
