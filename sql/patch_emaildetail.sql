-- Function: func_email_detail(bigint)

-- DROP FUNCTION func_email_detail(bigint);
-- 'MAIL_TEXT.TXT'를 제외하는 조건을 넣으면 안됨, Agent에서 본문보기를 할 수 없음

CREATE OR REPLACE FUNCTION func_email_detail(IN pemailseq bigint)
...
return query 
	 ...
	 WITH TBL_EMAIL_TRANSFER AS 
	( 
		SELECT * 
		FROM TBL_EMAIL_TRANSFER_HIS WHERE EMAIL_SEQ=pemailseq 
		UNION ALL 
		SELECT * 
		FROM TBL_EMAIL_TRANSFER_INFO WHERE EMAIL_SEQ=pemailseq
	) 
	, TBL_EMAIL_APPROVE AS 
	( 
		SELECT APPROVE_KIND AS APPROVEKIND, * 
		FROM   TBL_EMAIL_APPROVE_HIS WHERE EMAIL_SEQ=pemailseq 
		UNION ALL 
		SELECT '0' AS APPROVEKIND, * 
		FROM   TBL_EMAIL_APPROVE_INFO WHERE EMAIL_SEQ=pemailseq 
	) 
	, TBL_EMAIL_USER AS 
	( 
		SELECT * 
		FROM TBL_EMAIL_RECEIVER WHERE EMAIL_SEQ=pemailseq 
	) 
	, TBL_USERINFO AS
	(
	...
	(
	  SELECT 'APPROVE', 
		  AU.APPROVE_ORDER||'|'||U.USER_ID||'|'||U.USER_NAME||'|'||U.USER_RANK||'|'||AU.APPROVE_FLAG||'|'||AU.APPR_RES_TIME||'|' 
		  ||(CASE WHEN AU.APPROVE_DESC IS NULL THEN '' ELSE AU.APPROVE_DESC END) 
	...
	WHERE F.EMAIL_SEQ=pemailseq 
		AND F.ADD_TYPE='0'
	GROUP BY F.EMAIL_SEQ 

	UNION ALL 

	SELECT 'FILE',(F.FILE_SEQ||'|'||F.DLP||'|'||F.FILE_NAME||'|'||F.FILE_KIND||'|'||F.FILE_SIZE||'|'||(CASE WHEN VIRUS_FLAG='1' THEN VIRUS_DESC ELSE '-' END)) || '|'||'-'
	FROM TBL_EMAIL_ADD_FILE F 
	 LEFT OUTER JOIN TBL_EMAIL_VIRUS_HIS V ON F.EMAIL_SEQ=V.EMAIL_SEQ AND F.FILE_SEQ=V.FILE_SEQ AND V.VIRUS_FLAG='1'
	WHERE F.EMAIL_SEQ=pemailseq
		AND F.ADD_TYPE='0'
